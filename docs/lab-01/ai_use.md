...
| 9 | "ทำ Issue 3 เลย (Create & Seed Categories)" | สร้าง branch `feature/3-category-seed`, นิยาม `Category` model ใน `schema.prisma`, เขียน idempotent seed script ใน `seed.ts` ด้วย `upsert` | รัน `npx prisma generate` และ `npm run build` ผ่านสมบูรณ์ |
